...
while type(device) is dict:
    parameters = ','.join(device.keys())
    parameter =   raw_input("Enter parameter name ({}): ".format(parameters)).lower()
    print device.get(parameter, 'Такого параметра нет')
    break
else:
    print "Device with name \'{}\' not found. ".format(device_name)
